...
- Selected chunk actions: commit selected chunk, or commit + push selected chunk
- Push progress panel with completed / remaining push count
- Commit-only safety checkpoint with option to discard all local commits created in that session
...
def make_default_state():
    return {
        ...
        "improvement_notes": [
            "Confirm before destructive git actions such as reset --hard.",
            "Keep per-chunk logs so failed chunks can be audited later.",
            "Allow retry failed chunks without reprocessing completed chunks.",
            "Show branch/remote before starting so the user pushes to the correct target.",
            "Keep dry-run and visualize-only workflows for validation before touching Git."
        ],
        "stats": {
            "total": 0,
            "pending": 0,
...
def remove_processed_chunks(path, chunk_numbers):
    p = Path(path)
    if not p.exists():
        return
    processed = load_processed_chunks(p)
    for chunk_number in chunk_numbers:
        try:
            processed.discard(int(chunk_number))
        except Exception:
            pass
    p.write_text(json.dumps(sorted(processed), indent=2), encoding="utf-8")


def set_push_progress(**updates):
    with STATE_LOCK:
        STATE["push_progress"].update(updates)
        total = int(STATE["push_progress"].get("total") or 0)
        completed = int(STATE["push_progress"].get("completed") or 0)
        remaining = max(total - completed, 0) if total else int(STATE["push_progress"].get("remaining") or 0)
        STATE["push_progress"]["remaining"] = remaining
        STATE["push_progress"]["percent"] = int((completed / total) * 100) if total else 0
    save_state()


def remember_commit_only_checkpoint(repo):
    """Save the HEAD that commit-only runs can safely reset back to."""
    current = get_state_copy()
    if current.get("commit_only_session", {}).get("base_head"):
        return
    code, out = run_cmd(["git", "rev-parse", "HEAD"], cwd=repo)
    if code == 0 and out.strip():
        with STATE_LOCK:
            STATE["commit_only_session"] = {"base_head": out.strip(), "commits": [], "chunks": []}
        save_state()
        add_console(f"Commit-only checkpoint saved at {out.strip()[:12]}.", "info")


def remember_commit_only_commit(repo, chunk_number):
    code, out = run_cmd(["git", "rev-parse", "HEAD"], cwd=repo)
    if code != 0:
        return
    commit_hash = out.strip()
    with STATE_LOCK:
        session = STATE.setdefault("commit_only_session", {"base_head": "", "commits": [], "chunks": []})
        if commit_hash and commit_hash not in session["commits"]:
            session["commits"].append(commit_hash)
        if int(chunk_number) not in session["chunks"]:
            session["chunks"].append(int(chunk_number))
    save_state()
...
def process_worker(config):
    ...
    try:
        ...
        set_push_progress(total=0, completed=0, remaining=0, current=None, status="idle")
        if config["mode"] == "commit_only":
            remember_commit_only_checkpoint(Path(config["repo_path"]))

        if config["mode"] == "push_only":
            add_console("Push-only mode selected. Running git push once.", "info")
            set_push_progress(total=1, completed=0, remaining=1, current="branch", status="pushing")
            with STATE_LOCK:
                STATE["chunks"] = []
                recalc_stats_unlocked()
            save_state()
            ...
            if code == 0:
                set_push_progress(completed=1, remaining=0, current=None, status="done")
                add_console("Push completed successfully.", "success")
            else:
                set_push_progress(current=None, status="failed")
        ...
        save_state()
        if config["mode"] == "full":
            push_total = sum(1 for c in chunks if c.get("status") == "pending")
            set_push_progress(total=push_total, completed=0, remaining=push_total, current=None, status="waiting")
        ...
        for chunk in chunks:
            ...
            if config["mode"] == "commit_only":
                remember_commit_only_commit(repo, n)
            ...
            set_push_progress(current=n, status="pushing")
            ...
            save_processed_chunk(config["processed_chunks_file"], n)
            pp = get_state_copy().get("push_progress", {})
            set_push_progress(completed=int(pp.get("completed") or 0) + 1, current=None, status="done")
...
def start_selected_chunk(payload, selected_mode):
    """Run a normal worker but limit it to one selected chunk."""
    chunk_number = clean_pasted_path(str(payload.get("chunk_number", "") or ""))
    if not chunk_number.isdigit() or int(chunk_number) < 1:
        return False, "Select a valid chunk first."
    payload = dict(payload)
    payload["mode"] = selected_mode
    payload["start_chunk"] = chunk_number
    payload["end_chunk"] = chunk_number
    return start_processing(payload)


def discard_commit_only_commits(payload):
    current = get_state_copy()
    if current.get("running"):
        return False, "Stop processing before discarding commits."

    repo_path = clean_pasted_path(payload.get("repo_path", "") or current.get("repo_path", ""))
    processed_path = clean_pasted_path(payload.get("processed_chunks_file", "") or current.get("processed_chunks_file", ""))
    session = current.get("commit_only_session", {})
    base_head = session.get("base_head", "")
    chunks = session.get("chunks", [])

    if not repo_path or not Path(repo_path).is_dir() or not (Path(repo_path) / ".git").exists():
        return False, "Git repository folder not found."
    if not base_head:
        return False, "No commit-only checkpoint found. Nothing to discard."

    repo = Path(repo_path)
    add_console(f"Discarding commit-only session. Resetting to {base_head[:12]}...", "warning")
    code, out = run_cmd(["git", "reset", "--hard", base_head], cwd=repo, stream=True)
    if code != 0:
        update_state(last_error="Discard commit-only commits failed.")
        return False, "git reset --hard failed. Check the console."

    if processed_path:
        remove_processed_chunks(processed_path, chunks)

    with STATE_LOCK:
        STATE["commit_only_session"] = {"base_head": "", "commits": [], "chunks": []}
        for c in STATE.get("chunks", []):
            if c.get("number") in chunks:
                c["status"] = "pending"
                c["stage"] = "discarded_commit_only"
                c["progress"] = 0
        recalc_stats_unlocked()
    save_state()
    inspect_git(repo_path)
    add_console("Commit-only commits discarded successfully.", "success")
    return True, "Commit-only commits discarded."
...
.push-panel{margin-top:12px;background:#0b1220;border:1px solid var(--line);border-radius:10px;padding:10px}.push-row{display:flex;gap:12px;flex-wrap:wrap;color:#cbd5e1;font-size:13px}.danger-outline{background:#3f1111;border:1px solid #7f1d1d}.danger-outline:hover{background:#7f1d1d}
...
<header>
  ...
  <div class="toolbar">
    ...
    <button class="secondary" onclick="commitSelectedChunk()">Commit Selected Chunk</button>
    <button class="success" onclick="commitPushSelectedChunk()">Commit + Push Selected Chunk</button>
    <button class="danger-outline" onclick="discardCommitOnlyCommits()">Discard Commit-Only Commits</button>
  ...
  <div class="push-panel">
    <div class="push-row">
      ...
      <span id="pushStatus">Idle</span>
      <span>Completed: <b id="pushCompleted">0</b></span>
      <span>Remaining: <b id="pushRemaining">0</b></span>
      <span>Current: <b id="pushCurrent">-</b></span>
    </div>
    <div class="progress-wrap"><div class="progress-bar" id="pushBar"></div></div>
  </div>
  <div class="help">Tip: auto refresh pauses while you select/copy text. Use “Pause Auto Refresh” for long copy work. Commit-only mode saves a reset checkpoint so you can discard commits created by this tool before pushing.</div>
</header>
...
async function startProcessing(){
  const res = await api("/start", commonPayload());
  if (!res.ok) alert(res.message || "Failed to start");
  await refreshState(true);
}

async function commitSelectedChunk(){
  if (!selectedChunk) return alert("Select a chunk first.");
  const payload = commonPayload();
  payload.chunk_number = selectedChunk;
  const res = await api("/commit_selected", payload);
  if (!res.ok) alert(res.message || "Failed to commit selected chunk");
  await refreshState(true);
}

async function commitPushSelectedChunk(){
  if (!selectedChunk) return alert("Select a chunk first.");
  const payload = commonPayload();
  payload.chunk_number = selectedChunk;
  const res = await api("/commit_push_selected", payload);
  if (!res.ok) alert(res.message || "Failed to commit + push selected chunk");
  await refreshState(true);
}

async function discardCommitOnlyCommits(){
  const session = state?.commit_only_session || {};
  const count = (session.commits || []).length;
  if (!count) return alert("No commit-only commits recorded for discard.");
  const ok = confirm(`This will run git reset --hard back to the checkpoint and discard ${count} local commit(s) created in commit-only mode. Continue?`);
  if (!ok) return;
  const res = await api("/discard_commit_only", commonPayload());
  if (!res.ok) alert(res.message || "Failed to discard commits");
  await refreshState(true);
}
...
function updateStats(){
  ...
  const pp = state.push_progress || {};
  document.getElementById("pushStatus").textContent = pp.status || "idle";
  document.getElementById("pushCompleted").textContent = pp.completed ?? 0;
  document.getElementById("pushRemaining").textContent = pp.remaining ?? 0;
  document.getElementById("pushCurrent").textContent = pp.current ? `Chunk #${pp.current}` : "-";
  document.getElementById("pushBar").style.width = (pp.percent || 0) + "%";
}
...
function updateDetailStable(force=false){
  ...
  detail.innerHTML = `
    ...
    <div class="toolbar" style="margin:12px 0">
      <button class="secondary" onclick="commitSelectedChunk()">Commit This Chunk</button>
      <button class="success" onclick="commitPushSelectedChunk()">Commit + Push This Chunk</button>
    </div>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed.path == "/commit_selected":
            payload = self._read_json()
            ok, msg = start_selected_chunk(payload, "commit_only")
            return self._send_json({"ok": ok, "message": msg})

        if parsed.path == "/commit_push_selected":
            payload = self._read_json()
            ok, msg = start_selected_chunk(payload, "full")
            return self._send_json({"ok": ok, "message": msg})

        if parsed.path == "/discard_commit_only":
            payload = self._read_json()
            ok, msg = discard_commit_only_commits(payload)
            return self._send_json({"ok": ok, "message": msg})
