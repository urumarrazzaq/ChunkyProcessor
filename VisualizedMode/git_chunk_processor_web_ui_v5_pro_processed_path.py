...
- processed_chunks.json, or the path selected in UI
- logs/
"""
...
DEFAULT_PROCESSED_JSON = SCRIPT_DIR / "processed_chunks.json"
...
def load_processed_chunks(processed_json: Path) -> set:
...
def save_processed_chunks(processed_json: Path, done: set) -> None:
...
def add_event(meta: Dict, message: str) -> None:
    events = meta.setdefault("events", [])
    events.append({"time": now_text(), "message": str(message)})
    del events[:-250]
...
def build_state(chunks: List[Chunk], meta: Dict) -> Dict:
    ...
    return {
        ...
        "processed_chunks_file": meta.get("processed_chunks_file", ""),
...
def process_single_chunk(chunk: Chunk, repo_path: Path, chunks: List[Chunk], meta: Dict, processed: set, processed_json: Path) -> None:
    ...
    add_event(meta, f"Chunk #{chunk.number}: started")
    ...
    add_event(meta, f"Chunk #{chunk.number}: adding {len(existing_files)} files")
    ...
    add_event(meta, f"Chunk #{chunk.number}: committing")
    ...
    if push:
        ...
        add_event(meta, f"Chunk #{chunk.number}: pushing")
    ...
    save_processed_chunks(processed_json, processed)
    add_event(meta, f"Chunk #{chunk.number}: done")
...
def worker_process(config: Dict) -> None:
    ...
    meta = {
        ...
        "processed_chunks_file": str(clean_path(str(config.get("processed_chunks_file") or DEFAULT_PROCESSED_JSON))),
        "events": [],
    }
    ...
    try:
        ...
        processed_json = clean_path(str(config.get("processed_chunks_file") or DEFAULT_PROCESSED_JSON))
        if not processed_json.name:
            processed_json = DEFAULT_PROCESSED_JSON
        processed_json.parent.mkdir(parents=True, exist_ok=True)
        meta["processed_chunks_file"] = str(processed_json)
        add_event(meta, f"Using processed chunks file: {processed_json}")
        start_chunk = config.get("start_chunk")
        end_chunk = config.get("end_chunk")
        ...
        processed = load_processed_chunks(processed_json)
        for chunk in chunks:
        ...
        for chunk in chunks:
            ...
            save_state(chunks, meta)
            process_single_chunk(chunk, repo_path, chunks, meta, processed, processed_json)
            if pause > 0:
                time.sleep(pause)
...
.chunkGrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(58px,1fr));gap:8px;padding:12px;align-items:stretch}
.chunkCell{height:52px;min-width:0;width:100%;display:flex;align-items:center;justify-content:center;border-radius:12px;border:1px solid #263449;background:#0b1220;color:#dbeafe;font-weight:900;font-size:14px;letter-spacing:.15px;padding:0;position:relative;overflow:hidden;box-shadow:inset 0 -3px 0 rgba(255,255,255,.03)}
.chunkCell:hover{background:#12213a;border-color:#60a5fa;transform:translateY(-1px)}
.chunkCell.selected{background:#1e3a8a;border-color:#93c5fd;box-shadow:0 0 0 2px rgba(147,197,253,.22)}
.chunkCell.active{background:linear-gradient(135deg,#0e7490,#2563eb);border-color:#67e8f9;color:#ffffff;box-shadow:0 0 22px rgba(34,211,238,.35)}
.chunkCell.active::after{content:"";position:absolute;top:6px;right:6px;width:8px;height:8px;border-radius:50%;background:#22c55e;box-shadow:0 0 12px #22c55e}
.chunkCell.failed{border-color:#fb7185;background:#3f1020;color:#fecdd3}.chunkCell.success{border-color:#10b981;background:#062f27;color:#bbf7d0}.chunkCell.skipped{border-color:#f59e0b;background:#33240c;color:#fde68a}.chunkCell.dry_run{border-color:#c084fc;background:#32124d;color:#e9d5ff}.chunkCell.processing{border-color:#60a5fa;background:#112b55;color:#dbeafe}
.chunkNo{position:relative;z-index:1}.chunkMiniProg{position:absolute;left:0;bottom:0;height:4px;background:linear-gradient(90deg,#60a5fa,#34d399);width:0%}
.detailPanel{position:sticky;top:96px}.cardTop.active{box-shadow:inset 5px 0 0 #22d3ee}.consolePanel{margin-top:16px}.console{background:#020617;border:1px solid var(--border);border-radius:14px;padding:12px;max-height:220px;overflow:auto;font-family:Consolas,monospace;font-size:12px;color:#cbd5e1;white-space:pre-wrap}.etaLine{color:#cbd5e1;font-size:13px;margin-top:8px}
...
<section class="panel">
  <div class="grid">
    <div class="field"><label>git_chunks.log path</label><div class="pathRow"><input id="logFile" placeholder='D:\UE World\Project\git_chunks.log'><button class="secondary mini" onclick="browseLogFile()">Browse</button><button class="secondary mini" onclick="clearField('logFile')">Clear</button></div><div class="hint">Paste path, drag/drop path text, or click Browse to pick the .log file.</div></div>
    <div class="field"><label>Git repository folder path</label><div class="pathRow"><input id="repoPath" placeholder='D:\UE World\Project'><button class="secondary mini" onclick="browseRepoFolder()">Browse</button><button class="secondary mini" onclick="clearField('repoPath')">Clear</button></div><div class="hint">Use the folder that contains <b>.git</b>. Click Browse to select the repo root folder.</div></div>
    <div class="field"><label>processed_chunks.json location</label><div class="pathRow"><input id="processedFile" placeholder='C:\Users\Dev\Downloads\processed_chunks.json'><button class="secondary mini" onclick="browseProcessedFile()">Browse</button><button class="secondary mini" onclick="clearField('processedFile')">Clear</button></div><div class="hint">Pick where resume/progress history should be saved. Leave empty to save beside this script.</div></div>
  ...
  <div id="dropZone" class="drop" style="margin-top:12px">Tip: You can drag/drop copied path text here or directly into the fields. Browse buttons use the local Python app to safely pick files/folders on your PC.</div>
  <div class="actions"><button onclick="startProcessing()">Start Processing</button><button class="secondary" onclick="refreshState()">Refresh</button><button class="secondary" onclick="openStateFile()">Open State JSON</button><button class="secondary" onclick="openProcessedFile()">Open Processed JSON</button><button class="danger" onclick="stopProcessing()">Stop After Current Chunk</button></div>
...
<section class="panel consolePanel"><h2>Live Console</h2><div class="etaLine" id="etaLine">ETA: -</div><div id="consoleRoot" class="console">Waiting for events...</div></section>
</div>
<script>
let STATE = null;
...
async function browseLogFile(){const res=await api('/api/browse_log',{}); if(res.ok && res.path){$('logFile').value=res.path;} if(res.message){$('topMessage').textContent=res.message;}}
async function browseRepoFolder(){const res=await api('/api/browse_repo',{}); if(res.ok && res.path){$('repoPath').value=res.path;} if(res.message){$('topMessage').textContent=res.message;}}
async function browseProcessedFile(){const res=await api('/api/browse_processed',{}); if(res.ok && res.path){$('processedFile').value=res.path;} if(res.message){$('topMessage').textContent=res.message;}}
async function startProcessing(){const mode=$('mode').value; USER_SELECTED=false; SELECTED_CHUNK=null; const payload={log_file:$('logFile').value,repo_path:$('repoPath').value,processed_chunks_file:$('processedFile').value,start_chunk:$('startChunk').value?Number($('startChunk').value):null,end_chunk:$('endChunk').value?Number($('endChunk').value):null,pause:Number($('pause').value||0),dry_run:mode==='dryRun',push:mode==='push'}; $('topMessage').textContent='Starting...'; const res=await api('/api/start',payload); $('topMessage').textContent=res.message||JSON.stringify(res); refreshState();}
...
async function openProcessedFile(){await api('/api/open_processed',{});}
...
function render(){if(!STATE)return; const s=STATE.summary||{}; const counts=s.counts||{}; const complete=(counts.success||0)+(counts.skipped||0)+(counts.dry_run||0); $('totalChunks').textContent=s.total_chunks||0; $('totalFiles').textContent=s.total_files||0; $('doneChunks').textContent=complete; $('failedChunks').textContent=counts.failed||0; $('activeChunk').textContent=STATE.active_chunk?('#'+STATE.active_chunk):'-'; $('overallFill').style.width=(s.overall_pct||0)+'%'; $('topMessage').textContent=(STATE.app_status||'idle')+' — '+(STATE.message||'Ready')+' | '+(STATE.repo_path||''); if(STATE.processed_chunks_file && !$('processedFile').value){$('processedFile').value=STATE.processed_chunks_file;} renderEta(); renderConsole(); renderTable(); renderDetail();}
function renderEta(){if(!STATE){return;} const chunks=STATE.chunks||[]; const done=chunks.filter(c=>['success','skipped','dry_run'].includes(c.status)).length; const running=chunks.find(c=>c.status==='processing'); const started=chunks.filter(c=>c.started_at).length; $('etaLine').textContent=`Progress: ${done}/${chunks.length||0} chunks complete ${running?`• Active: #${running.number} (${running.stage}, ${running.progress_pct||0}%)`:''}`;}
function renderConsole(){const events=STATE?.events||[]; const root=$('consoleRoot'); if(!events.length){root.textContent='Waiting for events...';return;} root.textContent=events.slice(-120).map(e=>`[${e.time}] ${e.message}`).join('\n'); root.scrollTop=root.scrollHeight;}
function renderTable(){const root=$('chunkTableRoot'); const rows=visibleChunks(); $('shownCount').textContent=`${rows.length} chunks visible`; if(!rows.length){root.innerHTML='<div class="empty">No chunks match the current search/filter.</div>'; return;} root.innerHTML=`<div class="chunkGrid">${rows.map(c=>{const active=Number(STATE.active_chunk)===Number(c.number); const selected=Number(SELECTED_CHUNK)===Number(c.number); const status=String(c.status||'pending'); return `<button type="button" title="Chunk #${esc(c.number)} • ${esc(status)} • ${Number(c.progress_pct||0)}% • ${(c.files||[]).length} files" class="chunkCell ${esc(status)} ${active?'active ':''}${selected?'selected':''}" onclick="selectChunk(${Number(c.number)},true)"><span class="chunkNo">#${esc(c.number)}</span><span class="chunkMiniProg" style="width:${Number(c.progress_pct||0)}%"></span></button>`}).join('')}</div>`;}
...
def empty_state() -> Dict:
    return {
        ...
        "repo_path": "",
        "log_file": "",
        "processed_chunks_file": str(DEFAULT_PROCESSED_JSON),
        "events": [],
        "dry_run": False,
        "push": True,
        "active_chunk": None,
...
def open_file_dialog(filetypes=None) -> str:
    ...
    try:
        ...
        selected = filedialog.askopenfilename(title="Select git_chunks.log", filetypes=filetypes or [("All files", "*.*")])
        root.destroy()
        return selected or ""
    except Exception as exc:
        logging.error("Browse file dialog failed: %s", exc)
        return ""
...
def open_folder_dialog() -> str:
    ...
    try:
        ...
        root.withdraw()
        root.attributes("-topmost", True)
        selected = filedialog.askdirectory(title="Select Git repository folder")
        root.destroy()
        return selected or ""
    except Exception as exc:
        logging.error("Browse folder dialog failed: %s", exc)
        return ""


def open_processed_file_dialog() -> str:
    """Ask where processed_chunks.json should live. Existing or new file is okay."""
    try:
        import tkinter as tk
        from tkinter import filedialog
        root = tk.Tk()
        ...
        selected = filedialog.asksaveasfilename(
            title="Select or create processed_chunks.json",
        ...
        logging.error("Processed JSON dialog failed: %s", exc)
        return ""
    ...
    def do_POST(self):
        ...
        if length:
            try:
                ...
                payload = {}
        parsed = urlparse(self.path)
        if parsed.path == "/api/browse_log":
            path = open_file_dialog(filetypes=[("Log files", "*.log"), ("Text files", "*.txt"), ("All files", "*.*")])
            self.send_json({"ok": bool(path), "path": path or "", "message": "Selected log file." if path else "No file selected."})
            return
        if parsed.path == "/api/browse_repo":
            path = open_folder_dialog()
            self.send_json({"ok": bool(path), "path": path or "", "message": "Selected repo folder." if path else "No folder selected."})
            return
        if parsed.path == "/api/browse_processed":
            path = open_processed_file_dialog()
            self.send_json({"ok": bool(path), "path": path or "", "message": "Selected processed_chunks.json location." if path else "No processed JSON location selected."})
            return
        if parsed.path == "/api/start":
        ...
        elif parsed.path == "/api/open_processed":
            with APP_LOCK:
                path_text = (APP_STATE or {}).get("processed_chunks_file") or str(DEFAULT_PROCESSED_JSON)
            path = Path(path_text)
            ...
            try:
                ...
                pass
            self.send_json({"ok": True, "message": str(path)})
        else:
            self.send_json({"ok": False, "message": "Not found"}, 404)
